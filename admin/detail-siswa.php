 ...
 * Updated: Fix Mapel Pilihan (Rumpun + Manual), Remove Print Button
 */
$pageTitle = 'Detail Siswa'; // Default title
...
// 5. AMBIL MAPEL PILIHAN (FIXED LOGIC: GABUNGAN RUMPUN + MANUAL)
// A. Ambil dari Paket Rumpun (Jika ada)
$rumpunMapels = [];
if (!empty($siswa['kode_rumpun'])) {
    ...
    foreach ($rumpunRaw as $r) {
        $rumpunMapels[] = $r['nama_mapel'];
    }
}

// B. Ambil dari Pilihan Manual (Tabel siswa_mapel_pilihan)
$manualMapelsRaw = $db->query("
...
// C. Gabungkan & Grouping per Tingkat
$mapelByTingkat = [];

// Masukkan Mapel Rumpun (Diasumsikan berlaku untuk Kelas 11 & 12 / Fase F)
if (!empty($rumpunMapels)) {
    // Karena rumpun biasanya berlaku sepanjang Fase F (Kelas 11 & 12)
    // Kita masukkan ke kategori umum "Fase F (Paket Rumpun)"
    foreach ($rumpunMapels as $rm) {
        $mapelByTingkat['Paket Rumpun (' . $siswa['kode_rumpun'] . ')'][] = $rm;
    }
}

// Masukkan Mapel Manual (Sesuai Tingkat di Database)
foreach ($manualMapelsRaw as $m) {
    $tingkatLabel = "Pilihan Tambahan (Kelas " . $m['tingkat'] . ")";
    
    // Cek duplikasi dengan rumpun agar rapi (opsional)
    if (!in_array($m['nama_mapel'], $rumpunMapels)) {
        $mapelByTingkat[$tingkatLabel][] = $m['nama_mapel'];
    }
}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-6">
                ...
                <?php if (empty($mapelByTingkat)): ?>
                    <p class="text-sm text-slate-500 italic">Belum ada mapel pilihan yang diambil.</p>
                <?php else: ?>
                    <div class="space-y-4">
                        <?php foreach ($mapelByTingkat as $tingkat => $mapels): ?>
                            <div>
                                <span class="text-xs font-bold text-slate-400 uppercase block mb-2"><?= $tingkat ?></span>
